...
常见的命名法有三种：驼峰命名法、unix like命名法和匈牙利命名法。
...
原始的匈牙利命名法，现在被称为匈牙利应用命名法，由1972年至1981年在施乐帕洛阿尔托研究中心工作的程序员查尔斯·西蒙尼发明。
...
匈牙利命名法在标识符名称基础上加了一个或多个小写字母的前缀来说明数据类型，基本原则是变量名=属性+类型+对象描述，例如i16MaxValue表示16位有符号整型变量，m_lpszStr表示指向一个以0字符结尾的字符串的长指针成员变量。

匈牙利命名法来自于微软，也分为系统型匈牙利命名法和应用型匈牙利命名法。
...
**【规则】常量名所有字母全大写，不管是宏常量还是*const*常量。**
...
**【规则】采用小驼峰法，变量名第一个字母小写，后面每个单词首字母大写，其余均小写。**
...
**【规则】函数表示行为或动作，函数名应以动词开头，动词或动词+宾语。**
