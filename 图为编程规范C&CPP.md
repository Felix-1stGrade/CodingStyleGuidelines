...
作为一个软件开发人员，或多或少都会被别人甚至自己的糟糕代码绊倒过，如果编程不止两三年，也有可能被这种代码拖过后腿，导致项目进度延迟。糟糕的代码如同一团乱麻，每一处修改都影响到其它两三处，每一次新的需求加入，每一次问题的解决，都需要对这堆代码了然于心，这样才能继续往上堆代码，这团乱麻会越来越大，团队的生产力持续下降。

我们为糟糕的代码所困扰，怎么办？我们需要好的代码，需要易读、可维护、可靠、可测试、高效、可移植的代码。

编程规范即是对软件开发人员编写代码提出一些要求，以约束代码的使用和编写风格。希望通过规范来帮助或指导软件开发人员写出好的代码。

编程规范中的各种条目并不是毫无理由的提出，编写组分析总结了各种典型的编码问题，并参考业界编程规范，制定本规范。
...
**【原则】关系密切的代码靠近，按思路用空行分隔代码，函数调用者在被调用者上面。**

你是否曾经在某个类中摸索，从一个函数跳到另一个函数，想要弄清楚这些函数如何操作、如何互相相关，最后却被搞糊涂了？你是否曾经苦苦追索某个变量或函数的继承链条？这让人沮丧，因为你想要理解系统做什么，却花时间和精力在找到和记住那些代码碎片在哪里。所以，关系密切的代码应相互靠近，更不要把关系密切的代码放在不同的文件中。
...
v = w*x + y/z;  // Good：低优先级的二元操作符前后加空格，高优先级的不加空格
...
这段代码编程者的本意是应该有两种主要情况，x等于0以及x不等于0。对于x等于0的情形，除非y也等于0，否则程序不做任何处理；对于x不等于0的情形，程序将x与y之和赋值给z。

然而，这段代码实际上所做的与编程者的本意相去甚远，原因在于C语言中有这样的规则，else始终与同一对括号内最近的未匹配的if结合。那么代码实际执行的逻辑如下
...
**【原则】标识符命名应当简洁明确，名副其实，可望文知意。**

标识符名称最好采用英文单词的组合，不要用汉语拼音，更不要中英混合，用词应当简单准确。

标识符名称应告诉读者变量/函数/类型等做什么事，为什么存在，该怎么用，如果名称还需要注释来补充，那就不算名副其实。

```C++
int d; // 不好的命名，名称什么也没说明
int elapsedTimeInDays; // 好的命名，说明消逝的时间，以天计

int table[10]; // 不好的命名，table没有说明数组的用途，可以说是无意义的命名
```
...
名称中不要包含数据的类型，例如变量的类型变了，而没有同步更改变量名称中所包含的类型信息，必然会误导读者。

避免使用多个不同之处较小的名称。例如下面两个名称，如果在程序的不同地方出现，会很难区分出来的。
...
**【原则】以同样的方式拼写同样的概念（即每个概念对应一个词），拼写前后保持一致。**

例如对象属性值的获取和设置统一使用get/set。不要在不同的类中分别用同义词fetch、get来表示同样的概念。
...
**【规则】对于常见缩略词，两个或更少字母的标识符要采用全大写或全小写的方式；多字母缩略词采用大驼峰法或小驼峰法。**
...
**【规则】禁止无意义的字母或字母加数字命名，禁止使用小写字母l和大写字母O命名。**
...
- pX是指向另一个**X**类型的指针，这包含非常少的语义信息。
- d是一个前缀表示两个值的差，例如，**dY**可能代表一个图形沿Y轴的距离，而一个仅仅叫做**y**的变量可能是一个绝对坐标。这完全是自然语义的。
- sz是一个无结束或零结束的字符串。在C中，这包含一些语义信息，因为C语言的**char\***类型的变量不确定是一个指向单个字符的指针，还是一个字符数组，或是一个零结束字符串。
- w标记一个变量是一个字。这基本上没有包含什么语义信息，因此可以被看作一种系统匈牙利命名法。
- b标记了一个字节，和w对比可能有一些语义信息，因为C语言中，只有**char**型（以及signed/unsigned char)是一个字节长的，这些类型有时候被用来保存数值或字符。这个前缀可以明确某个变量保存的是字符还是数值。
...
- 匈牙利命名法在被用作代表多个属性的时候会造成困惑，如 a_crszkvc30LastNameCol：一个常量[参数]，保存了一个[varchar]（30）类型的[数据库]列LastName的内容，而这列又是这个表的[主键]的一部分。
...
.NET Framework，微软新的软件开发平台，除了接口类型一般不适用匈牙利命名法。在.NET中，习惯在接口类型前放一个“I”（例如Windows Forms中的IButtonControl接口。）.NET Framework指导方针建议程序员不要用匈牙利命名法，但是没有指明不要用系统匈牙利命名法还是匈牙利应用命名法，或者是两者都不要用。与此对比，Java的标准库中连接口类型也不加前缀。
...
**【规则】私有变量，局部变量，函数参数的名称采用小驼峰法，第一个字母小写，后面每个单词首字母大写，其余均小写。**
...
**【建议】在变量名的末尾追加计算限定符（Avg、Sum、Min、Max、Index等）。**

**【规则】禁止单字母作为变量名，除i，j，k作为局部循环变量外(单字符的变量名一般只用于生命期非常短暂的变量)。**
...
加前缀m\_后，当变量在public和private间更改时，会带来修改变量名称的工作量。

此外，人们看代码时也会无视前缀或后缀，只看到名称中有意义的部分，所以m\_并没有多少意义。
...
长而具有描述性的名称，要比短而令人费解的名称好，长而具有描述性的名称，要比描述性的长注释好。命名方式要保持一致，使用与模块名一脉相承的短语、名词和动词给函数命名。
...
一般的，尽量通过清晰的架构逻辑，好的符号命名来提高代码可读性；仅在需要的时候，才辅以注释说明，并且要时刻谨记，注释不能美化糟糕的代码，真正好的注释是想办法优化代码而不需写注释。 
...
当需要写注释的时候，不妨再思考一下，能否通过优化代码逻辑，或者更改变量/函数命名来避免写注释。

**【原则】注释要简洁明确，且在功能、意图层次上解释，着重说明“为什么”。**
...
**【原则】注释不是逐行描述代码，不要写废话，不要写日志，不要有误导性信息。**
...
**【建议】每一个`.cpp`/`.c`文件应有一个对应的.h文件，用于声明需要对外公开的类与接口。**
...
**【建议】头文件包含顺序：首先是`.cpp`相应的`.h`文件，其它头文件按照稳定度排序。**
...
**【规则】只能通过包含头文件的方式使用其它模块提供的接口，禁止在`.c`/`.cpp`中通过`extern`的方式使用外部函数接口、变量。**
...
是F(x)=((x)-1)吗？不是的。实际上定义的是F=(x) ((x)-1)，因为在F后面多了一个空格。
...
**【规则】不应使用基本数据类型`char`、`int`、`short`、`long`、`float`和`double`，而应使用`typedef`重新进行类型的定义，并标明数据长度和符号。**
...
避免滥用typedef或者#define对基本类型起别名，并且在C++11以后推荐使用using来定义类型。
...
**【规则】不要使用 #define 进行数据类型别名定义。**

宏定义只是文本的替换，有时会出现意想不到的错误。例如：
...
**【规则】禁止`signed`类型与`unsigned`类型之间的隐式转换。**

有符号数转换成无符号数可能导致符号丢失，无符号数转为有符号数可能得到一个负值。

**【规则】禁止`int`类型与`float`类型之间的隐式转换。**

浮点型与整型之间转换会导致精度丢失。

**【建议】后缀`U`/`u`应该用在所有`unsigned`类型的常量上。**
...
有时候在上下文中为了格式对齐的需要，可能无意中将十进制数写成了八进制数，例如
...
**【规则】每个外部变量只能定义一次，并且只在一个地方声明，即在头文件中声明，需要用到该外部变量的所有模块都应包含这个头文件。**
...
C/C++中有数十个运算符，优先级与结合律如下表。

![运算符的优先级与结合律](.\picture\运算符的优先级与结合律.png)

注意一元运算符的优先级高于对应的二元运算符。在二元运算符中，算术运算符的优先级最高，移位运算符次之，关系运算符再次之，接着是逻辑运算符，赋值运算符。
...
6个关系运算符的优先级并不相同。==和!=的优先级要低于其它关系运算符。

任何两个逻辑运算符都具有不同的优先级，所有按位运算符的优先级比顺序运算符的优先级高，每个“与”运算符要比相应的“或”运算符优先级高，而按位异或运算符的优先级介于按位与和按位或之间。
...
**【注意】C语言中只有四个运算符（&&、||、?:和,）存在规定的求值顺序，其它所有运算符对其操作数的求值顺序是未定义的。特别地，赋值运算符并不能保证任何求值顺序。**

例如，要对a<b求值，编译器可能先对a求值，也可能先对b求值，在某些机器上甚至有可能对它们同时并行求值。

例如，y[i]=x[i++]; 就是不正确的，因为C语言实现上y[i]有可能在i自增前求值，也可能相反。
...
**【注意】对变量进行移位操作应注意是否产生了溢出或数据截断。**

如果被移位的对象的长度是n位，那么移位计数必须大于或等于0，而且小于n。因此，不可能在单次操作中将某个数值的所有位都移出。

举例来说，如果一个int型整数是32位，n是一个int型整数，那么n<<31和n<<0这样写是合法的，而n<<32和n<<-1是非法的。

需要注意的是，即使C实现将符号位复制到空出的位中，有符号整数的向右移位运算也并不等同于除以2的某次幂。让我们考虑(-1)>>1，这个移位操作的结果一般不可能为0，但是(-1)/2在大多数C实现上求值结果都是0。
...
**【规则】判断指针变量是否为空，应将指针变量与`NULL`进行相等或不等的比较，而不是与0比较。**
...
**【规则】每一个`case`分支的结尾必须有`break`语句，否则匹配一个`case`后会继续执行下一个`case`分支，直到遇到`break`语句或者`switch`结束（除非编码的目的本就是多`case`连续执行）。**
...
**【规则】`switch`必须有`default`分支。**

**【建议】为了提高程序效率，应把`switch`-`case`语句中的每一种具体情况按照它们发生的相对频率排序，把最可能发生的情况放在最前面，最不可能发生的情况放在最后。**
...
**【规则】禁止在`for`循环体内修改循环变量，防止`for`循环失控。**
...
**【建议】如果函数的参数是bool型，可考虑根据true和false取值拆分函数。**
...
带有“记忆”功能的函数，其行为可能是不可预测的，因为它的行为可能取决于某种“记忆状态”。这样的函数既不易理解又不利于测试和维护。在 C/C++语言中，函数的static 局部变量是函数的“记忆”存储器。**建议尽量少用 static局部变量，除非必需**。
...
**【规则】函数的参数要书写完整，不要只写参数类型不写参数名字，如无参数必须写`void`。**
...
**【规则】对于引用类型的形参，如果是不需要修改的，请使用`const`。**

把const作为默认选项，在编译时会对其进行检查，使代码更牢固/更安全。

**【规则】对于不会修改成员变量的成员函数请使用`const`修饰。**
...
参数一般会被自然而然的看作是函数的输入，因此，尽量避免使用输出参数。如果的确需要输出参数，可以考虑用修改所属对象成员变量的方式代替。
...
**【规则】非`void`类型函数，在每个出口都应有一个含有返回值的`return`语句，并且返回值与函数返回值类型一致。**

**【规则】不要将正常值和错误标志混在一起返回。正常值用输出参数获得，而错误标志用`return`语句返回。**
...
指针是指针，数组是数组，指针和数组之间没有任何关系。

指针变量在32位系统下，永远占4个字节，其值为某一个内存的地址，指针可以指向任何地方，但不是任何地方都能通过这个指针变量访问到。

数组的大小与元素的类型和个数有关，定义数组时必须指定元素的类型和个数，数组可以存任何类型的数据，但不能存函数。

对于一个数组，只能做两件事：确定数组的大小，以及获得指向该数组下标为0的元素的指针。其它有关数组的操作，看上去是以数组下标进行运算的，实际上都是通过指针进行的。
...
如果array不是用于sizeof的操作数（ sizeof(array)的结果是整个数组的大小，其值为sizeof(INT16)*10，此处array代表数组 ），在其它场合array总是被转换成一个指向array数组首元素的指针。

**【注意】数组名作为sizeof的操作数时代表整个数组，其它场合都是数组首元素的指针。**
...
**【规则】对于传递到函数的指针参数，应判断是否为`NULL`。**

**【规则】调用返回类型为指针的函数后，应判断返回值是否为`NULL`。**
...
**【规则】使用`malloc`等函数申请内存时，必须对分配失败做异常处理。**
...
**【规则】单个对象释放使用`delete`，数组对象释放使用`delete []`。**
...
**【规则】字符串连接宜使用`strncat`库函数代替`strcat`库函数，字符串拷贝已使用`strncpy`库函数代替`strcpy`库函数，避免长度不够引起的数组越界。**

**【规则】使用`sprintf`库函数时，应注意字符长度，避免长度不够引起的数组越界。**
...
**【规则】不应在中断程序中使用`printf`、`malloc`等函数。**
...
| 序号 | 检查项   | 检查说明                           | 是否合格 |
| ---- | -------- | ---------------------------------- | -------- |
| 1    | 数组 | 检查操作数组元素是否超出了最大下标 |          |
| 2    | 指针     | 使用指针前，指针是否初始化         |          |
| 3    | 指针 | 不再使用的指针是否赋值为NULL       |          |
| 4    | 数据类型         | 有符号整型和无符号整型比较存在风险 |          |
| 5    | 宏 | 宏表达式必须有完备的括号 |          |
| 6    | 溢出 | 整数乘法是否溢出 |          |
| 7    | 硬件 | 比较器，周期等外设存储器必须初始化 |          |
| 8    | 变量记忆清除 | 对于一些与历史数据有关的计算，如数字滤波器、积分等算法，在初始或者重新开始计算时必须考虑对历史数据的清除 |          |
| 9    |          |                                    |          |
|  | | | |
|  | | | |
|  | | | |
